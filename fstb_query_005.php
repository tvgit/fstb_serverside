...
echo "<HTML><HEAD></HEAD><BODY>";
    ...
    $res = $db->query("SELECT DISTINCT date(datum), esp8266id FROM fstb ORDER BY datum, esp8266id");

    if ($res){
      // echo '<form action="fstb_show_001.php" method="post">';
      echo '<form action="fstb_plot_005.htm" method="post">';
      ...
      while ($_record = $res -> fetchArray(SQLITE3_ASSOC)){
        $_datum_str = $_record["date(datum)"];
        $_esp8266id_str = $_record["esp8266id"];
        ...
        if ($_datum_str != ''){
			if ($_datum_str != $_datum_str_old){
                ...
                make_checkbox_cell($_esp8266id_str, "esp8266id");
                echo '&emsp; &emsp;';
                make_form_cell();
